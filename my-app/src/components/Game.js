...
export default function Game() {
  ...
  const onCardPlayedHandlerForPlayer = (played_card) => {
      switch (played_card) {
          ...
          case "d4W": {
              const numberOfPlayedCard = 'd4W';
                  ...
                  if (uppercasedColor && allowedKeys.includes(uppercasedColor)) {
                      ...
                      setDrawCardPile(copiedDrawCardPileArray);
                      const updatedBotDeck = [...botDeck, drawCard1, drawCard2, drawCard3, drawCard4];
                      setBotDeck(updatedBotDeck);
                  }else {
                      alert('Invalid Key!');
                  }
          ...
          case 'W': {
            ...
            const uppercasedColor = newColor.toUpperCase();
            if (uppercasedColor === 'R' || uppercasedColor === 'G' || uppercasedColor === 'B' || uppercasedColor === 'Y') {
                playWildCardSound();
                handleCardPlay(played_card);
  ...
  const onCardDrawnHandler = () => {
    ...
    for (let i = 0; i < playerDeck.length; i++) {
     ...
     if (color === currentColor || number === currentNumber ||
        number === 'W' || number === 'd') 
        {
        return;
    }
    ...
    if (numberOfDrawnCard === currentNumber || colorOfDrawnCard === currentColor) {
        ...
        if (colorOfDrawnCard === currentColor || numberOfDrawnCard === currentNumber) {
...
const findMostRepeatedColor = () => {
    ...
    setTimeout(() => {
        ...
        if (ColorAndNumberOfCard > 0) {
            ...
            for (let i = 0; i < ColorAndNumberOfCard; i++) {
                ...
                if ((colorOfCurrentCard === currentColor || numberOfCurrentCard === currentNumber)) {
        ...
        const botPlayedCardForColorAndNUmber = botPlayedCardIndexColorAndNumber !== null ? colorAndNumberCards[botPlayedCardIndexColorAndNumber] : null;
        const numberOfBotPlayedCard = botPlayedCardForColorAndNUmber ? botPlayedCardForColorAndNUmber.charAt(0) : null;
        const colorOfBotPlayedCard = botPlayedCardForColorAndNUmber ? botPlayedCardForColorAndNUmber.charAt(1) : null;
        ...
        if (NumberOfDrawCards > 0) {
            ...
            for (let i = 0; i < NumberOfDrawCards; i++) {
                ...
                if (colorOfCurrentCardDraw2 === currentColor || numberOfCurrentCardDraw2 === currentNumber) {
        ...
        const numberOfBotPlayedCardForWild = botPlayedCardForWild ? botPlayedCardForWild.charAt(0) : null;
        ...
        const skipCardPlay = (cardToPlay, numberOfCardToPlay , colorOfCardToPlay) =>{
            setPlayedCardsPile((prevPile) => [...prevPile, cardToPlay]);
            setBotDeck((prevDeck) => {
                const indexToRemove = prevDeck.indexOf(cardToPlay);
                if (indexToRemove !== -1) {
        ...
        const colorAndNumberCardPlay = (cardToPlay, numberOfCardToPlay , colorOfCardToPlay) =>{
            setPlayedCardsPile((prevPile) => [...prevPile, cardToPlay]);
            setBotDeck((prevDeck) => {
                const indexToRemove = prevDeck.indexOf(cardToPlay);
                if (indexToRemove !== -1) {
 ...
 setBotDeck((prevDeck) => {
        ...
        const draw4WildCardPlay = (cardToPlay, numberOfCardToPlay) =>{
            ...
            const drawCard4 = copiedDrawCardPileArray.pop();
            setDrawCardPile(copiedDrawCardPileArray);
            setPlayerDeck((prevDeck) => [...prevDeck, drawCard1, drawCard2, drawCard3, drawCard4]);
            playDraw4CardSound();
            setTurn('Bot');
        }


        if (NumberOfReverseCards > 0 && (colorOfBotPlayedCardForReverse === currentColor || numberOfBotPlayedCardForReverse === currentNumber)) {
            reverseCardPlay(botPlayedCardForReverse , numberOfBotPlayedCardForReverse,  colorOfBotPlayedCardForReverse)
          ...
          ;
            ...
            if (/^[0-9]+[RGBY]$/.test(drawCard1) && ( currentColor === colorOfBotDrawnCard || currentNumber === numberOfBotDrawnCard)){
                colorAndNumberCardPlay(drawCard1 , numberOfBotDrawnCard , colorOfBotDrawnCard)
            }
            else if (/^D2[RGBY]$/.test(drawCard1) &&  (currentColor === colorOfBotDrawnCard || currentNumber === numberOfBotDrawnCard)){
                drawCardPlay(drawCard1 , numberOfBotDrawnCard , colorOfBotDrawnCard)
            }
            else if (/^reve[RGBY]$/.test(drawCard1) && (currentColor === colorOfBotDrawnCard || currentNumber === numberOfBotDrawnCard)){
                reverseCardPlay(drawCard1 , numberOfBotDrawnCard , colorOfBotDrawnCard)
            }
            else if (/^skip[RGBY]$/.test(drawCard1) && (currentColor === colorOfBotDrawnCard || currentNumber === numberOfBotDrawnCard)){
              ...
              draw4WildCardPlay(drawCard1 , numberOfBotDrawnCard , colorOfBotDrawnCard)
            }
            else {
                setTurn('Player')
            }
  ...
  return (
        <>
               {/* PlayedCardsPile */}
      <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', paddingRight: '550px' ,paddingTop: '220px',}}>
          <div style={{
    position: 'relative',
    backgroundColor:
      currentColor === 'G' ? '#33b84b' :
      currentColor === 'B' ? '#0098ff' :
      currentColor === 'Y' ? '#ffc63c' :
      currentColor === 'R' ? '#ff242c' : 'transparent',
    width: '115px',
    height: '155px',
    borderRadius:'10px',
  }}>
         ...
         src={require(`../assets/${playedCardsPile[playedCardsPile.length - 1]}.png`)}  />
              )}
          </div>
      </div>
      ...
      {/* BotDeck */}
      <div style={{  position: 'fixed', top: '10px',right: '10px', padding: '5px 5px', alignItems: 'center', }}>
          {botDeck.map((item, card , index) => (
          <img key={`${card}-${index}`} style={{width: '100px', height: 'auto'}}
...
{gameOver && (
  <div id="winBox" style={{
    ...
    <h2>{winner === 'Player' ? 'CONGRATULATIONS!' : 'Better luck next time!'}</h2>
